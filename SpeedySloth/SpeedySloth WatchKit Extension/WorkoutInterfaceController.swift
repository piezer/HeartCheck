...
class WorkoutInterfaceController: WKInterfaceController, HKWorkoutSessionDelegate {
    ...
    let targetHeart: Int = mainInstance.heartRate
    //heartrate stuff
    ...
    func updateLabels() {
        ...
        if self.alertShow == false && self.currentHeartRate > targetHeart{
            showPopup()
            self.alertShow = true
            if let session = workoutSession{
                healthStore.pause(_: session)
        ...
        if self.alertShow == true && self.currentHeartRate < targetHeart{
    ...
    func showPopup(){
        let h0 = {print("Slow Down")}
        let action1 = WKAlertAction(title: "Okay", style: .default, handler:h0)
        presentAlert(withTitle: "Slow Down! Your heart rate is too high!", message: "", preferredStyle: .actionSheet, actions: [action1])
    ...
    func createHeartRateStreamingQuery(_ workoutStartDate: Date) -> HKQuery?{
        ...
        self.updateHeartRate(_samples: sampleObjects)
        }
        
        heartRateQuery.updateHandler = {(query, samples, deleteObjects, newAnchor, error) -> Void in
            self.updateHeartRate(_samples: samples)
        }
        return heartRateQuery
    }
    
    func updateHeartRate(_samples: [HKSample]?){
        guard let heartRateSamples = _samples as? [HKQuantitySample] else {return}
        if isPaused == false{
            DispatchQueue.main.async {
                guard let sample = heartRateSamples.first else{return}
                let value = sample.quantity.doubleValue(for: self.heartRateUnit)
                self.currentHeartRate = Int(value)
                self.heartLabel.setText(String(UInt16(value))+" BPM")
            }
        }
    }
